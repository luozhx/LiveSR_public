...
class BitrateAdaptor:
    ...
    def select_action(self, buffer_size, file_size, download_rate, sr_gains) -> int:
        ...
        next_video_chunk_sizes.append(MODEL_SIZE)
        ...
        state[0, 5, -1] = self.last_gain - 1.0

        action = self.net.select_action(state)
        if action >= BITRATE_LEVELS:
            quality = action - BITRATE_LEVELS
        else:
            quality = action

        # --linear reward--
        gain = sr_gains[quality]  # from client, real time update
        bitrate = gain * VIDEO_BIT_RATE[quality]
